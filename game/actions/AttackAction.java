 ...
 * Modified by: Joanne Yew, Incy Lin
 *
 */
	...
	public String execute(Actor actor, GameMap map) {
		...
		if (!target.isConscious()) {
			if (target instanceof Enemy) {
				...
				//Transfer runes
				if (actor.toString() == "Tarnished") {
					RuneManager runeManager = new RuneManager();
					int a = RandomNumberGenerator.getRandomInt(((Enemy) target).getMin_runes(), ((Enemy) target).getMax_runes());
					System.out.println(enemy);
					runeManager.AddRunes(actor, a);

					result += System.lineSeparator() + target + " transfers " + a + " runes\n";
				}
				//Enemy dies
